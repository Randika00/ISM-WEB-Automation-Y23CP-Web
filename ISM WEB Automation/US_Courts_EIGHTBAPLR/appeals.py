...
sheet_links = {
    'The Criminal Appeals': 'https://www.ca8.uscourts.gov/rules-procedures'
}
    ...
    for i in p_element:
        ...
        if link:
            ...
            if not pdf_url.startswith(('http://', 'https://')):
                pdf_url = 'https://www.ca8.uscourts.gov/' + pdf_url
            try:
                pdf_response = requests.get(pdf_url)
                if pdf_response.status_code == 200:
                    title_element = i.find('strong')
                    if title_element:
                        title = title_element.text.strip()
                    else:
                        title = 'Rules & Procedures'

                    title = remove_invalid_paths(title)

                    output_fileName = ret_file_name_full(source_id, location_id, title, '.pdf')
